...
import { ArrowLeft, Baby, LogOut, Mail, Pencil, Plus, Share2, Trash2, UserPlus, Users } from 'lucide-react-native';
import { NativeStackScreenProps } from '@react-navigation/native-stack';

import { useAuth } from '../context/AuthContext';
import { useFamily } from '../context/FamilyContext';
import { addChild, createInvitation, deleteChild, updateChild, updateFamilyName, updateParentProfile } from '../services/familyService';
...
function parseDate(value: string) {
  ...
  const now = new Date();
  return { year: parts?.[0] || now.getFullYear(), month: (parts?.[1] || now.getMonth() + 1) - 1, day: parts?.[2] || now.getDate() };
}
...
export default function ProfileScreen({ navigation }: Props) {
  ...
  function openEditChild(child: Child) { setEditingChild(child); setChildForm({ full_name: child.full_name ?? '', date_of_birth: child.date_of_birth ?? '', school_name: child.school_name ?? '', notes: child.notes ?? '' }); setChildOpen(true); }
  ...
  async function createInvite() {
    ...
    if (!inviteEmail.includes('@')) return Alert.alert('Email required', 'Enter your co-parent’s email.');
    setSaving(true);
    try {
      const invite = await createInvitation(familyId, inviteEmail, user.id);
      const link = `forthemapp://accept-invite?token=${invite.token}`;
      setInviteLink(link);
      await Share.share({ title: 'Join ForThem', message: `Join me on ForThem for ${familyName || 'our co-parenting space'}:\n${link}` });
    } catch (e: any) { Alert.alert('Error', e?.message ?? 'Could not create invite.'); }
    finally { setSaving(false); }
  }
  ...
  async function saveChild() {
    ...
    try {
      ...
      if (editingChild) await updateChild(editingChild.id, payload); else await addChild(familyId, payload);
      await refresh(); setChildOpen(false);
  ...
  function confirmDeleteChild() {
    ...
    Alert.alert('Delete child profile?', `This removes ${editingChild.full_name} from ForThem. Documents stay in the Documents tab.`, [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => { setSaving(true); try { await deleteChild(editingChild.id); await refresh(); setChildOpen(false); } catch (e: any) { Alert.alert('Error', e?.message ?? 'Could not delete child.'); } finally { setSaving(false); } } },
    ]);
  }

  function handleSignOut() { Alert.alert('Sign out?', 'You can sign back in anytime.', [{ text: 'Cancel', style: 'cancel' }, { text: 'Sign out', style: 'destructive', onPress: signOut }]); }

  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        <View style={styles.topbar}><TouchableOpacity style={styles.circleBtn} onPress={() => navigation.goBack()}><Icon icon={ArrowLeft} size="sm" color={Colors.textPrimary} strokeWidth={2} /></TouchableOpacity><Text style={styles.title}>Profile</Text><View style={styles.circleGhost} /></View>

        <Card style={styles.profileCard}><View style={styles.avatar}><Text style={styles.avatarText}>{firstLetter(displayName)}</Text></View><View style={styles.flex1}><Text style={styles.name}>{displayName}</Text><View style={styles.rowSmall}><Icon icon={Mail} size="xs" color={Colors.textSecondary} strokeWidth={1.8} /><Text style={styles.muted}>{email}</Text></View>{!!phone && <Text style={styles.muted}>{phone}</Text>}</View><TouchableOpacity style={styles.iconBtn} onPress={() => setProfileOpen(true)}><Icon icon={Pencil} size="sm" color={Colors.primary} strokeWidth={1.9} /></TouchableOpacity></Card>

        <SectionHeader title="Co-parenting space" style={styles.section} />
        <TouchableOpacity activeOpacity={0.8} onPress={() => setSpaceOpen(true)}><Card style={styles.rowCard}><View style={styles.softIcon}><Icon icon={Users} size="md" color={Colors.primary} strokeWidth={1.8} /></View><View style={styles.flex1}><Text style={styles.cardTitle}>{familyName || 'Co-parenting space'}</Text><Text style={styles.muted}>{childCountLabel}</Text></View><Icon icon={Pencil} size="sm" color={Colors.textSecondary} strokeWidth={1.8} /></Card></TouchableOpacity>

        <SectionHeader title="Co-parent" style={styles.section} />
        <Card style={styles.rowCard}><View style={styles.softIcon}><Icon icon={UserPlus} size="md" color={Colors.primary} strokeWidth={1.8} /></View><View style={styles.flex1}><Text style={styles.cardTitle}>Invite co-parent</Text><Text style={styles.muted}>Share a secure invite link when you are ready.</Text></View><TouchableOpacity style={styles.smallPrimary} onPress={() => { setInviteLink(''); setInviteOpen(true); }}><Text style={styles.smallPrimaryText}>Invite</Text></TouchableOpacity></Card>
        ...
        <TouchableOpacity style={styles.signOutButton} onPress={handleSignOut}><Icon icon={LogOut} size="sm" color={Colors.error} strokeWidth={1.9} /><Text style={styles.signOutText}>Sign out</Text></TouchableOpacity><View style={{ height: Layout.screenPaddingB }} />
      </ScrollView>

      <EditSheet visible={profileOpen} title="Edit parent profile" onCancel={() => setProfileOpen(false)} onSave={saveParent} saving={saving}><Field label="NAME" value={nameDraft} onChangeText={setNameDraft} placeholder="Your name" /><Field label="PHONE OPTIONAL" value={phoneDraft} onChangeText={setPhoneDraft} placeholder="Phone number" keyboardType="phone-pad" /></EditSheet>
      <EditSheet visible={spaceOpen} title="Edit co-parenting space" onCancel={() => setSpaceOpen(false)} onSave={saveSpace} saving={saving}><Text style={styles.bodyText}>Use a neutral label that feels comfortable, like “Noah’s space”.</Text><Field label="SPACE NAME" value={spaceDraft} onChangeText={setSpaceDraft} placeholder="Co-parenting space" /></EditSheet>
      <EditSheet visible={inviteOpen} title="Invite co-parent" onCancel={() => setInviteOpen(false)} onSave={createInvite} saving={saving} saveLabel="Create link"><Text style={styles.bodyText}>Enter your co-parent’s email. ForThem will generate a secure invite link you can share.</Text><Field label="CO-PARENT EMAIL" value={inviteEmail} onChangeText={setInviteEmail} placeholder="their@email.com" keyboardType="email-address" autoCapitalize="none" />{!!inviteLink && <Text style={styles.linkText}>{inviteLink}</Text>}</EditSheet>
      <EditSheet visible={childOpen} title={editingChild ? 'Edit child profile' : 'Add child'} onCancel={() => setChildOpen(false)} onSave={saveChild} saving={saving}><Field label="CHILD NAME" value={childForm.full_name} onChangeText={(v) => setChildForm({ ...childForm, full_name: v })} placeholder="e.g. Noah" /><Text style={styles.label}>DATE OF BIRTH OPTIONAL</Text><TouchableOpacity style={styles.input} onPress={() => setDateOpen(true)}><Text style={childForm.date_of_birth ? styles.inputText : styles.placeholder}>{childForm.date_of_birth || 'Select date'}</Text></TouchableOpacity><Field label="SCHOOL OR DAYCARE OPTIONAL" value={childForm.school_name} onChangeText={(v) => setChildForm({ ...childForm, school_name: v })} placeholder="School or daycare name" /><Field label="NOTES OPTIONAL" value={childForm.notes} onChangeText={(v) => setChildForm({ ...childForm, notes: v })} placeholder="Medical notes, routines, preferences" multiline />{editingChild && <TouchableOpacity style={styles.deleteButton} onPress={confirmDeleteChild}><Icon icon={Trash2} size="sm" color={Colors.error} strokeWidth={2} /><Text style={styles.deleteText}>Delete child profile</Text></TouchableOpacity>}</EditSheet>
...
const styles = StyleSheet.create({
  ...
  rowCard: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md }, softIcon: { width: 48, height: 48, borderRadius: Radius.lg, backgroundColor: Colors.primary + '12', alignItems: 'center', justifyContent: 'center' }, smallPrimary: { backgroundColor: Colors.primary, borderRadius: Radius.full, paddingHorizontal: 14, paddingVertical: 9 }, smallPrimaryText: { color: '#FFFFFF', fontWeight: '800', fontSize: 12 }, cardTitle: { ...Typography.body, fontWeight: '800', color: Colors.textPrimary }, bodyText: { ...Typography.caption, color: Colors.textSecondary, lineHeight: 20, marginTop: 4 }, childCard: { flexDirection: 'row', alignItems: 'center', gap: Spacing.md, marginBottom: Spacing.sm }, childAvatar: { width: 48, height: 48, borderRadius: Radius.lg, backgroundColor: Colors.secondary + '18', alignItems: 'center', justifyContent: 'center' }, emptyCard: { gap: Spacing.sm },
