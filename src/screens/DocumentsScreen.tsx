...
type PlanState = 'missing' | 'uploaded' | 'analyzing';
...
function ParentingPlanCard({ state, onUpload, onView }: { state: PlanState; onUpload: () => void; onView: () => void }) {
  ...
  return <Card style={styles.planCard}>
    <View style={styles.planHeader}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={styles.planTitle}>Parenting Plan</Text>
        <Text style={styles.planSub}>{uploaded ? 'Johnson County Parenting Plan.pdf' : 'Not uploaded yet'}</Text>
      </View>
    ...
    {!uploaded ? <>
      ...
      <View style={styles.powerList}>
        {['Custody schedule', 'Expense splits', 'Handoff rules'].map(item => <View key={item} style={styles.powerItem}><Text style={styles.powerCheck}>✓</Text><Text style={styles.powerText}>{item}</Text></View>)}
      </View>
      <TouchableOpacity style={styles.planUploadBtn} onPress={onUpload} activeOpacity={0.85}><Text style={styles.planUploadBtnText}>Upload PDF</Text></TouchableOpacity>
    </> : <>
      <View style={styles.analysisBox}>
        <Text style={styles.analysisTitle}>{state === 'analyzing' ? 'Analyzing your plan...' : 'Ready for future automation'}</Text>
        <Text style={styles.analysisText}>{state === 'analyzing' ? 'Extracting custody schedule, expense rules, handoffs, and timelines.' : 'AI extraction will connect this document to Calendar and Expenses in a later slice.'}</Text>
      </View>
      <View style={styles.planActions}><TouchableOpacity style={styles.secondaryBtn} onPress={onView}><Text style={styles.secondaryBtnText}>View</Text></TouchableOpacity><TouchableOpacity style={styles.primarySmallBtn} onPress={onUpload}><Text style={styles.primarySmallBtnText}>Replace</Text></TouchableOpacity></View>
    </>}
  </Card>;
}

function PlanUploadSheet({ visible, onClose, onMockUpload }: { visible: boolean; onClose: () => void; onMockUpload: () => void }) {
  return <Modal visible={visible} transparent animationType="slide"><Pressable style={styles.overlay} onPress={onClose}><Pressable style={styles.sheet} onPress={e => e.stopPropagation()}><View style={styles.sheetHandle} /><Text style={styles.sheetTitle}>Upload Parenting Plan</Text><Text style={styles.sheetSub}>Choose a PDF of your official agreement. Storage comes in the next slice.</Text><View style={styles.sourceRow}><TouchableOpacity style={styles.sourceTile} onPress={onMockUpload}><Text style={styles.sourceEmoji}>📂</Text><Text style={styles.sourceLabel}>Files</Text></TouchableOpacity><TouchableOpacity style={styles.sourceTile} onPress={onMockUpload}><Text style={styles.sourceEmoji}>📷</Text><Text style={styles.sourceLabel}>Scan</Text></TouchableOpacity></View><TouchableOpacity style={styles.cancelBtn} onPress={onClose}><Text style={styles.cancelText}>Cancel</Text></TouchableOpacity></Pressable></Pressable></Modal>;
}
...
export default function DocumentsScreen() {
  ...
  const [planState, setPlanState] = useState<PlanState>('missing');
  ...
  function mockPlanUpload() { setPlanState('analyzing'); setPlanSheetOpen(false); showToast('📄 Parenting plan selected'); setTimeout(() => setPlanState('uploaded'), 1800); }
  const filtered = useMemo(() => { let list = docs; if (activeFolder) list = list.filter(d => d.category === activeFolder); if (query.trim()) { const q = query.toLowerCase(); list = list.filter(d => d.name.toLowerCase().includes(q) || d.category.toLowerCase().includes(q)); } return list; }, [docs, activeFolder, query]);
  ...
  return <SafeAreaView style={styles.safe} edges={['top']}>
    ...
    <PlanUploadSheet visible={planSheetOpen} onClose={() => setPlanSheetOpen(false)} onMockUpload={mockPlanUpload} />
  </SafeAreaView>;
}
...
const styles = StyleSheet.create({
  ...
  screenTitle: { ...Typography.h1 }, screenSub: { ...Typography.small, marginTop: 2 },
