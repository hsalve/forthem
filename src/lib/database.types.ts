...
export type EventCategory      = 'School' | 'Medical' | 'Daycare' | 'Activity' | 'Other';
...
export type ParentingPlanRules = {
  [category: string]: {
    payer_pct:   number;   // percentage the "payer" parent is responsible for
    partner_pct: number;
    notes?:      string;
  };
};
...
export type Swap = {
  ...
  family_id:    string;
  requested_by: string;
  giving_date:  string;     // ISO date "YYYY-MM-DD"
  wanting_date: string;
  reason:       string | null;
  note:         string | null;
  status:       SwapStatus;
...
export type InsertInvitation = {
  family_id:  string;
  invited_by: string;
  email:      string;
};

export type InsertParentingPlan = {
  family_id:    string;
  uploaded_by:  string;
  file_name?:   string;
  file_url?:    string;
...
export type InsertCustodySchedule = {
  family_id:       string;
  child_id:        string;
  parent_id:       string;
  start_date:      string;
...
export type Database = {
  public: {
    Tables: {
      ...
      parenting_plans: {
        Row:    ParentingPlan;
        Insert: InsertParentingPlan;
        Update: UpdateParentingPlan;
      };
      custody_schedule: {
        Row:    CustodySchedule;
        Insert: InsertCustodySchedule;
        Update: UpdateCustodySchedule;
      };
      calendar_events: {
        Row:    CalendarEvent;
